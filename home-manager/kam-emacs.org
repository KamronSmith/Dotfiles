...
#+end_src
*** The =Dired= section for =reay-player=
#+begin_src emacs-lisp :tangle (format user-init-file "%s")
...
(use-package vertico-multiform
  ...
  (setq vertico-multiform-categories
        `((embark-keybinding ,@kam-vertico-multiform-maximal)
...
With optional argument FRAME, return the list of buffers of FRAME."
    ...
    (defun kam-beframe-buffer-sort-recency (buffers)
      "Group the given BUFFERS by recency then sort them."
      (let ((bufs (seq-group-by
                   (lambda (buf)
                     (cond
                      ...
                      (t :hidden)))
                   buffers)))
        (nconc (alist-get :current bufs)
               (alist-get :visible bufs)
               (alist-get :hidden bufs))))
...
If the point is at the end of the line, join the next line to the current line. If the point is at either the beginning of the line or at the first text of the line, join the current line to the previous line."
  (interactive)
  (if (eolp)
      (progn
        (next-line)
...
(defun kam-set-font-faces ()
  ...
  (set-face-attribute 'variable-pitch nil :family "Iosevka Comfy Duo" :height .9 :weight 'regular)
...
(use-package eglot
  ...
  (defun kam-eglot-eldoc ()
      (setq eldoc-documentation-strategy 'eldoc-documentation-compose-eagerly))
  :hook ((eglot-managed-mode . kam-eglot-eldoc)
...
(defun kam-task-update-projects-list ()
  "Updates the project listing with all of the projects in the `kam-task-projects-directory'."
  (add-to-list 'project--list
               (directory-files kam-task-projects-directory))
  (seq-uniq project--list))

(defun kam-task-new-project ()
  ...
  (let* ((default-directory kam-task-projects-directory)
         ...
         (new-project
          (concat kam-task-projects-directory
    ...
    (write-region (concat "* " project-name) nil kam-task-inbox-file 'append)))

(defun kam-task-delete-project ()
  "Delete a project."
  (interactive)
  (let ((default-directory kam-task-projects-directory)
        (project (read-directory-name "Project to delete: ")))
    (delete-directory (concat kam-task-projectsa))))
...
(defun kam-consult-org-heading-link ()
  ...
  (kam-insert-last-stored-link-with-prompt))
...
(defun kam-modeline-major-mode-name ()
  ...
  (concat (nerd-icons-icon-for-mode major-mode) " " (capitalize (string-replace "-mode" "" (symbol-name major-mode)))))
...
(defun kam-modeline--lines-and-columns ()
  "Return the point's position in terms of lines and columns in the current buffer.")
